...
def addToList(self):
    ...
    if (len(var_fname)>0) and (len(var_lname)>0) and(len(var_phone)>0) and (len(var_email)>0): #enforces user to provide the names
        conn=sqlite3.connect('db_phonebook.db')
        with conn:
            cursor=conn.cursor()
    ...
    if (len(var_phone)>0 and (len(var_email)>0):
       ...
       with conn:
           cur=conn.cursor()
            ...
            print(count2)
            if count==0 or count2==0: #proposed changes are not in the database, then proceed
