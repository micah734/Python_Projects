...
def first_run(self):
    data=('Sally','Doe','John Doe','111-111-1111','jdoe@email.com','History')
    conn = sqlite3.connect('db_student.db')
    with conn:
        cur = conn.cursor()
...
def addToList(self):
    ...
    var_fname=var_fname.strip() #will remove blank spaces before or after user entry
    var_lname=var_lname.strip()
    var_fname=var_fname.title() #will ensure tath first letter in each word is capitalized
    var_lname-var_lname.title()
    var_full=("{} {}".format(var_fname,var_lname)) #combine into fullname
    ...
    if (len(var_fname)>0) and (len(var_lname)>0) and(len(var_phone)>0) and (len(var_email)>0 and (len(var_course)>0): #enforces user to provide the names
        conn=sqlite3.connect('db_student.db')
        with conn:
            cursor=conn.cursor()
            #check database for existance of the fullname
            ...
            if chkName==0:
                ...
                messagebo.showerror("Name Error","'{}' already exists in the database! Please choose a different name".format(var_fullname))
                onClear(self)
        conn.commit()
        conn.close()
    else:
...
def onDelete(self):
    ...
    with conn:
        ...
        #the databse..cannot delete last record or get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_student""")
        count=cur.fetchone()[0]
        if count>1:
...
def onUpdate(self):
    ...
    #for name changes must delete the entire record and start over
    var_phone=self.txt_phone.get().strip()#normalized the data
    var_email=self.txt_email.get().strip()
    var_course=self.txt_course.get().strip()
    if (len(var_phone)>0) and (len(var_email)>0) and (len(var_course)>0):
       ...
       with conn:
           cur=conn.cursor()
            #count records to see if users changes are already in
            #db..meaning, there are not changes to update
            ...
            print(count3)
            if count==0 or count2==0 or count3==0: #proposed changes are not in the database, then proceed
                response=messagebox.askokcancel("Update Request","The following changes ({}), ({}) and ({}) will be implemented for ({}). \n\nProceed with the update request?".format(var_phone,var_email,var_course,var_value))
                print(response)
                if response:
                    with conn:
                        cursor=conn.cursor()
                        cursor.execute("""UPDATE tbl_phonebook SET col_phone='{0}',col_email='{1}', col_course='{2}'  WHERE col_fullname='{3}'""".format(var_phone,var_email,var_course,var_value))
                        onClear(self)
                        conn.commit()
                else:
                    messagebox.showinfo("Cancel Request","No changes have been made to ({}).".format(var_value))
            else:
                messagebox.showinfo("No changes detected","Both ({}), ({}), ({}) \nalready exist in the database for this name. \n\nYour update request has been cancelled.".format(var_phone,var_email,var_course))
            onClear(self)
        conn.close()
    else:
